...
class MotionLoader:
    def __init__(
        ...
        body_names_in_motion_data, joint_names_in_motion_data = (
            self._load_data_from_motion_npz(motion_file, device)
        )
        body_indexes = self._get_index_of_a_in_b(
            robot_body_names, body_names_in_motion_data, device
        )
        joint_indexes = self._get_index_of_a_in_b(
            robot_joint_names, joint_names_in_motion_data, device
        )
    ...
    def _get_index_of_a_in_b(
        ...
        for name in a_names:
            if name in b_names:
                ...
                name in FALLBACK_BODY_NAME_ALIASES
                and FALLBACK_BODY_NAME_ALIASES[name] in b_names
            ):
                # Use fallback alias if original name not found but alias exists
                alias = FALLBACK_BODY_NAME_ALIASES[name]
                logger.debug(f"Using fallback alias: {name} -> {alias}")
                indexes.append(b_names.index(alias))
            else:
                raise AssertionError(
                    f"The specified name ({name}) doesn't exist: {b_names}"
                )
        return torch.tensor(indexes, dtype=torch.long, device=device)

    def _load_data_from_motion_npz(
        self, motion_file: str, device: str
        ...
        with cached_open(motion_file, "rb") as f, np.load(f) as data:
            ...
            self._joint_pos = torch.tensor(
                data["joint_pos"][:, 7:], dtype=torch.float32, device=device
            )
            self._joint_vel = torch.tensor(
                data["joint_vel"][:, 6:], dtype=torch.float32, device=device
            )
            assert (
                len(joint_names) == self._joint_pos.shape[1]
            ), "Joint names in motion data does not match"

            self._body_pos_w = torch.tensor(
                data["body_pos_w"], dtype=torch.float32, device=device
            )
            assert (
                len(body_names) == self._body_pos_w.shape[1]
            ), "Body names in motion data does not match"
    ...
    def extend_with_segments(
        self, segments: dict[str, torch.Tensor], prepend: bool
    ) -> MotionLoader:
        ...
        for seg_key, attr_name in concat_targets:
            ...
            tensors = (
                (segments[seg_key], existing)
                if prepend
                else (existing, segments[seg_key])
            )
...
class AdaptiveTimestepsSampler:
    ...
    def __init__(
        ...
        self.num_bins = math.ceil(
            (self.motion_time_step_total / self.env_fps) / self.bin_size_s
        )
    ...
    def init_buffers(self):
        self.current_bin_failed_count = torch.zeros(
            self.num_bins, dtype=torch.float, device=self.device
        )
    ...
    def update_current_bin_failed_count(self, failed_at_time_step: torch.Tensor):
        ...
        failed_bin = torch.floor(
            failed_at_time_step / self.motion_time_step_total * self.num_bins
        ).long()
        assert (
            failed_bin.min() >= 0 and failed_bin.max() < self.num_bins
        ), "Failed bin is out of range"
        self.current_bin_failed_count[:] = torch.bincount(
            failed_bin, minlength=self.num_bins
        )
    ...
    def sample(self, num_samples: int) -> torch.Tensor:
        sampled_bins = torch.multinomial(
            self.sampling_probabilities, num_samples, replacement=True
        )
...
FAKE_BODY_NAME_ALIASES: dict[str, str] = {
    ...
    # Walker specific - TCP links exist in URDF but not in motion data
    # TCP (Tool Center Point) is at the end of arm, closest real body is wrist
    "left_tcp_link": "wrist_roll_l_link",
    "right_tcp_link": "wrist_roll_r_link",
    # IMU is mounted on pelvis, which maps to Base_link in motion data
    "imu_link": "Base_link",
}

# Fallback aliases: used when original name is not found in motion data
# For example, Walker robot uses 'pelvis' in URDF but 'Base_link' in motion data
# Also handles naming convention differences: G1 uses "left_*" prefix, Walker uses "*_l" suffix
FALLBACK_BODY_NAME_ALIASES: dict[str, str] = {
    "pelvis": "Base_link",
    # G1 style -> Walker style (for foot contact point aliases)
    "left_ankle_roll_link": "ankle_roll_l_link",
    "right_ankle_roll_link": "ankle_roll_r_link",
}
...
class MotionCommand(CommandTermBase):
    def __init__(self, cfg: Any, env: WholeBodyTrackingManager):
        ...
        if isinstance(cfg.params["motion_config"], MotionConfig):
            ...
            self.motion_cfg = MotionConfig(**cfg.params["motion_config"])
        self.init_pose_cfg: NoiseToInitialPoseConfig = (
            self.motion_cfg.noise_to_initial_pose
        )
    ...
    def setup(self) -> None:
        ...
        robot_body_names_alias = [
            FAKE_BODY_NAME_ALIASES.get(bn, bn) for bn in robot_body_names
        ]
        ...
        self.ref_body_index = robot_body_names.index(
        ...
        if self.motion.has_object:
            ...
            self.object_indices_in_simulator = self._env.simulator.get_actor_indices(
                self.object_name, env_ids=None
            )

            assert (
                self._env.simulator.get_simulator_type() == SimulatorType.ISAACSIM
    ...
    def reset(self, env_ids: torch.Tensor | None) -> None:
        ...
        ) * 2 * root_ang_vel_noise_rpy.unsqueeze(
            0
        )  # (num_envs, 3)
    ...
    def step(self) -> None:
        ...
        robot_ref_pos_w = self.robot_root_pos_w * use_root + self.robot_ref_pos_w * (
            1 - use_root
        )
        robot_ref_quat_w = self.robot_root_quat_w * use_root + self.robot_ref_quat_w * (
            1 - use_root
        )
        ...
        self.body_quat_relative_w = quat_mul(
            delta_quat_w, self.body_quat_w, w_last=True
        )
    ...
    def root_pos_w(self) -> torch.Tensor:
        return (
            self.motion.body_pos_w[self.time_steps, 0]
            + self._env.simulator.scene.env_origins
        )
    ...
    def robot_body_quat_w(self) -> torch.Tensor:
        return self._env.simulator._rigid_body_rot[
    ...
    def simulator_object_pos_w(self) -> torch.Tensor:
        return self._env.simulator.all_root_states[self.object_indices_in_simulator][
            :, :3
        ]
    ...
    def init_buffers(self):
        ...
        self.body_pos_relative_w = torch.zeros(
            self.num_envs,
            len(self.motion_cfg.body_names_to_track),
            3,
            device=self.device,
    ...
    def update_metrics(self):
        ...
        self.metrics["motion/error_ref_pos"] = torch.norm(
            self.ref_pos_w - self.robot_ref_pos_w, dim=-1
        )
        self.metrics["motion/error_ref_rot"] = quat_error_magnitude(
        ...
        if self.motion_cfg.use_adaptive_timesteps_sampler:
            ...
            self.metrics["motion/adaptive_timesteps_sampler_entropy"] = (
                self.adaptive_timesteps_sampler.metrics["sampling_entropy"]
            )
            self.metrics["motion/adaptive_timesteps_sampler_top1_prob"] = (
                self.adaptive_timesteps_sampler.metrics["sampling_top1_prob"]
            )
            self.metrics["motion/adaptive_timesteps_sampler_top1_bin"] = (
                self.adaptive_timesteps_sampler.metrics["sampling_top1_bin"]
            )
    ...
    def _maybe_add_default_pose_transition(self, *, prepend: bool) -> None:
        ...
        enabled = (
            ...
            if prepend
            else self.motion_cfg.enable_default_pose_append
        )
        if not enabled:
            return
    ...
    def _build_default_pose_state(
        self, use_motion_end: bool = False
        ...
        init_root_quat = torch.tensor(
            init_state.rot, dtype=torch.float32, device=self.device
        ).unsqueeze(0)
        ...
        motion_root_quat = (
            self.motion.body_quat_w[motion_idx, 0].to(self.device).unsqueeze(0)
        )
        ...
        default_root_lin_vel = torch.tensor(
            init_state.lin_vel, dtype=torch.float32, device=self.device
        )
        default_root_ang_vel = torch.tensor(
            init_state.ang_vel, dtype=torch.float32, device=self.device
        )
        ...
        default_body_lin_vel = self._map_robot_bodies_to_motion_order(
            body_states["lin_vel"]
        )
        default_body_ang_vel = self._map_robot_bodies_to_motion_order(
            body_states["ang_vel"]
        )
    ...
    def _add_transition_to_motion(
        self, default_state: dict[str, torch.Tensor], num_steps: int, prepend: bool
    ) -> None:
        ...
        default_motion_state = self._default_motion_state(
            default_state, dtype=dtype, device=device
        )
        motion_state = self._motion_state(
            0 if prepend else -1, dtype=dtype, device=device
        )
    ...
    def _slerp_quat_sequence(
        self, start: torch.Tensor, end: torch.Tensor, alphas: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _capture_body_states(
        ...
        assert (
            simulator.get_simulator_type() == SimulatorType.ISAACSIM
    ...
    def _map_robot_bodies_to_motion_order(
        self, robot_tensor: torch.Tensor
    ) -> torch.Tensor:
        ...
        motion_tensor = torch.zeros(
            motion_shape, device=robot_tensor.device, dtype=robot_tensor.dtype
        )
    ...
    def _motion_state(
        self, idx: int, dtype: torch.dtype, device: torch.device
    ) -> dict[str, torch.Tensor]:
        ...
        state = {
            ...
            "body_lin_vel": self.motion._body_lin_vel_w[idx].to(
                device=device, dtype=dtype
            ),
            "body_ang_vel": self.motion._body_ang_vel_w[idx].to(
                device=device, dtype=dtype
            ),
        }
        if self.motion.has_object:
            state["object_pos"] = self.motion._object_pos_w[idx].to(
                device=device, dtype=dtype
            )
            state["object_quat"] = self.motion._object_quat_w[idx].to(
                device=device, dtype=dtype
            )
            state["object_lin_vel"] = self.motion._object_lin_vel_w[idx].to(
                device=device, dtype=dtype
            )
        return state
    ...
    def _default_motion_state(
        ...
        state = {
            ...
            "body_lin_vel": default_state["body_lin_vel"].to(
                device=device, dtype=dtype
            ),
            "body_ang_vel": default_state["body_ang_vel"].to(
                device=device, dtype=dtype
            ),
        }
        if self.motion.has_object:
            state["object_pos"] = default_state["object_pos"].to(
                device=device, dtype=dtype
            )
            state["object_quat"] = default_state["object_quat"].to(
                device=device, dtype=dtype
            )
            state["object_lin_vel"] = default_state["object_lin_vel"].to(
                device=device, dtype=dtype
            )
        return state
    ...
    def _build_transition_segments(
        ...
        segments = {
            ...
            "body_lin_vel": _lerp(
                start["body_lin_vel"], target["body_lin_vel"], alphas_body
            ),
            "body_ang_vel": _lerp(
                start["body_ang_vel"], target["body_ang_vel"], alphas_body
            ),
            "body_quat": self._slerp_quat_sequence(
                start["body_quat"], target["body_quat"], alphas
            ),
        }

        if self.motion.has_object:
            segments["object_pos"] = _lerp(
                start["object_pos"], target["object_pos"], alphas_joint
            )
            segments["object_lin_vel"] = _lerp(
                start["object_lin_vel"], target["object_lin_vel"], alphas_joint
            )
            segments["object_quat"] = self._slerp_quat_sequence(
                start["object_quat"].unsqueeze(0),
                target["object_quat"].unsqueeze(0),
                alphas,
            ).squeeze(1)

        return segments

    def _apply_transition_segments(
        self, segments: dict[str, torch.Tensor], prepend: bool
    ) -> None:
    ...
    def _build_and_apply_transition(
        ...
        alphas = torch.linspace(
            0.0, 1.0, steps=num_steps + 1, device=device, dtype=dtype
        )
        if drop_first:
            alphas = alphas[1:]
        if drop_last:
        ...
        segments = self._build_transition_segments(
            start_state, target_state, alphas, alphas_joint, alphas_body
        )
    ...
    def _setup_visualization_markers_for_isaacsim(self):
        ...
        for body_names in self.motion_cfg.body_names_to_track:
            ...
            visualization_markers_cfg.markers["hit"].visual_material.diffuse_color = (
                0.0,
                1.0,
                0.0,
            )
            self.visualization_markers[f"motion_{body_names}"] = VisualizationMarkers(
                visualization_markers_cfg
            )
