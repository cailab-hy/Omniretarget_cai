...
class MotionLoader:
    def __init__(
        ...
        robot_dof: int = 30,  # Added robot_dof parameter, default to 30 for Walker
    ):
    ...
    def _load_motion(self):
        ...
        # Use robot_dof to slice strictly the robot joints
        # Base (7) + Joints (robot_dof)
...
def run_simulator(args_cli: DataConversionConfig):
    ...
    # Force robot to walker if not consistent, or at least respect the walker config
    if args_cli.robot != "walker":
         print(f"Warning: Script intended for walker, but got {args_cli.robot}. Proceeding as walker.")
         args_cli.robot = "walker"
    ...
    # Introspect robot config to get DoF if possible, otherwise default to 30
    # The RobotConfig object usually has it but here we are before loading constants fully
    # However we can construct it.
    robot_config_temp = RobotConfig(robot_type="walker")
    robot_dof = robot_config_temp.robot_dof if hasattr(robot_config_temp, "robot_dof") else 30
    
    # But wait, RobotConfig might not have robot_dof attribute directly in some versions of code,
    # usually it's in constants. Let's trust the user knows this is for Walker (30 DoF).
    # Based on previous file, constants.ROBOT_DOF was accessed.
    ...
    motion = MotionLoader(
        ...
        robot_dof=30 # Hardcoded for Walker
    )
