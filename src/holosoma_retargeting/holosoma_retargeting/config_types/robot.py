...
_ROBOT_DEFAULTS: dict[str, RobotDefaults] = {
    ...
    "walker": {"robot_dof": 30, "robot_height": 1.7, "object_name": "ground"},
}
...
class RobotConfig:
    ...
    def _foot_sticking_links(self) -> list[str]:
        ...
        if self.robot_type == "walker":
            return [
                "ankle_roll_l_sphere_1_link",
                "ankle_roll_l_sphere_2_link",
                "ankle_roll_l_sphere_3_link",
                "ankle_roll_l_sphere_4_link",
                "ankle_roll_l_sphere_5_link",
                "ankle_roll_r_sphere_1_link",
                "ankle_roll_r_sphere_2_link",
                "ankle_roll_r_sphere_3_link",
                "ankle_roll_r_sphere_4_link",
                "ankle_roll_r_sphere_5_link",
            ]
    ...
    def _manual_lb(self) -> dict[str, float]:
        ...
        elif self.robot_type == "walker":
            # ankle_roll_l=12, ankle_roll_r=18 (limit side-to-side ankle movement)
            base.update(
                {
                    # "12": 0.0,  # ankle roll l (±6 degrees)
                    "18": 0.0,  # ankle roll r (±6 degrees)
                }
            )
    ...
    def _manual_ub(self) -> dict[str, float]:
        ...
        if self.robot_type == "g1":
            ...
            base.update(
                {
                    "12": 0,  # ankle roll l (±6 degrees)
                    # "18": 0,  # ankle roll r (±6 degrees)
                }
            )
    ...
    def _manual_cost(self) -> dict[str, float]:
        ...
        # if self.robot_type == "walker":
        #     return {"19": 0.2}  # body yaw (prevent excessive trunk rotation)
        return {}
    ...
    def _nominal_tracking_indices(self) -> np.ndarray:
        ...
        if self.robot_type == "h1":
            return np.arange(19)
        # if self.robot_type == "walker":
        #     # Include leg joints (0-11) + waist joint (12) for stability
        #     return np.arange(19)
